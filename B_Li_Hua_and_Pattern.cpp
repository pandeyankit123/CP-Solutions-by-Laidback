...
signed main(){
    ...
    while(T--){
        int n, k; cin>>n>>k;
        vector<vector<int>> v(n, vector<int> (n)), dv;
        fo(i,0,n) cinall(v[i]); dv=v;
        fo(i,0,n) reverse(all(dv[i]));
        reverse(all(dv)); int cnt=0;
        fo(i,0,n) fo(j,0,n) if(v[i][j]!=dv[i][j]) cnt++;
        cnt/=2; if(cnt>k) no;
		else{
			if(n%2) yes;
            else {k-=cnt; if(k%2) no; else yes;}
		}
    }
    return 0;
}
