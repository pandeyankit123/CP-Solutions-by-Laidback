...
signed main(){
    ...
    while(T--){
        ...
        fo(i,0,n) cin>>v[i], sum+=v[i], pre[i+1]=pre[i]+v[i];
        fo(i, 0, 101){
            int pre=0, f=1;
            fo(j, 0, n){
                int l=max(cal(v[j]*i), cal(sum*i)-pre);
                int r=min(cal(v[j]*(i+1)), cal(sum*(i+1))-pre)-1;
                if(l<=r){
                    coutn(i); f=0;
                    break;
                } if(f) pre+=v[j]; else break;
            }
        }
    } return 0;
}
