...
signed main(){
    ...
    while(T--){
        ...
        int cnt0=0, sp=0, sn=0;
        fo(i,0,n){
            cin>>v[i];
            if(v[i]>0) pos.pb(v[i]), sp+=v[i];
            else if(v[i]==0) cnt0++;
            else neg.pb(v[i]), sn+=v[i];
        }if(neg.size()%2 && !cnt0){
            if(pos.size()){
                sort(all(pos)); sort(all(neg));
                if(abs(neg.back())>pos[0])
                    coutn((sp-sn)-(2*pos[0]));
                else coutn(sp-sn+(2*neg.back()));
            }else coutn(abs(sn)+(2*(*max_element(all(neg)))));
        }else coutn(sp-sn);
    }
    return 0;
}
