...
signed main(){
    ...
    while(T--){
        ...
        fo(i,0,n){
            ...
            int val; cin>>val;
            v.pb({val,1});
        } cinall(wt); sort(all(v)); sort(all(wt));
        fo(i,0,2*n){
            if(v[i].ss) diff.pb(v[i].ff-st.top()), st.pop();
            else st.push(v[i].ff);
        } sort(all(diff), greater<int> ()); int ans=0;
        fo(i,0,n) ans+=wt[i]*diff[i]; coutn(ans);
    } return 0;
}
