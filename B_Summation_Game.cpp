...
signed main(){
    ...
    while(T--){
        ...
        fo(i,1,n) pre[i]=pre[i-1]+v[i];
        int ans, hi=n-1, lo=hi-x+1;
        if(lo<=0) ans=-1*pre[hi];
        else ans=2*pre[lo-1]-pre[hi];
        fo(i,max(n-k, 0ll),n){
            if(i==0){ 
                ans=max(ans,0LL); 
                continue; 
            } hi=i-1; lo=hi-x+1; 
            if(lo<=0) ans=max(ans,-1*(pre[hi]));
            else ans=max(ans, 2*pre[lo-1]-pre[hi]);
        } coutn(ans);
    } return 0;
}
